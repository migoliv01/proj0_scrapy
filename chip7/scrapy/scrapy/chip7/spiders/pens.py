...
from chip7.items import Pens
from chip7.settings import NAME_PENS, URL_PENS, MAX_PAGES

# seletor dos cards
CARD_SEL = '.flex.flex-col.flex-grow.px-2.pb-2'

class PensSpider(scrapy.Spider):
    name   = NAME_PENS
    ...
    def _make_request(page, callback):
        ...
        return scrapy.Request(
            url=f'{URL_PENS}?category_page={page}',
            meta={
            ...
            callback=callback,
            cb_kwargs={'page': page},
            dont_filter=True,  # para evitar que nao faca algum url (pode acontecer retornar ao mesmo url e Scrapy skip auto, isto faz que nao skip qualquer url duplicado(q n acontece) mas é só para ficar mais seguro)
        )
    ...
    def _parse_card(card):
        ...
        return Pens(
            ...
            descricao       = card.css('p.text-gray-500.swiper-no-swiping::text').get(),
